...
async function promptText(opts: Parameters<typeof text>[0]): Promise<string> {
  ...
  if (isCancel(result)) { cancel('Aborted.'); process.exit(0); }
  return result;
}

async function promptSelect<T extends string>(opts: { message: string; options: Array<{ value: T; label: string }> }): Promise<T> {
...
export async function runOnboarding(defaults: { apiKey?: string; adapter?: 'v3' | 'v4'; model?: string }): Promise<OnboardingResult> {
  ...
  const apiKey = defaults.apiKey ?? await promptText({
    message: 'DeepSeek API key',
    placeholder: 'sk-...',
    validate: (v) => (v.length < 20 ? 'API key looks too short' : undefined),
  });

  const adapter = defaults.adapter ?? await promptSelect<'v3' | 'v4'>({
    message: 'Adapter',
    options: [
